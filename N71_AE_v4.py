...
def ETL_Downtime(resD, db):
    ...
    Standard_List = ["time mode", "err code", "datetime", "ae no.", "hw rev.", "sw rev.", "description", "color code"]
    ...
    for dicts in resD:
        ...
        if "downtime_error" not in station_sub[stationSub]:
            ...
            station_sub[stationSub]["downtime_error"][''] = {'count': 0}
            station_sub[stationSub]["downtime_error"]["lastRec"] = []
            
        if tmp['err code'] not in station_sub[stationSub]["downtime_error"]:
#            if tmp['time mode'] == '0':
#                station_sub[stationSub]["downtime_error"]["lastRec"].append((tmp['datetime'], tmp['time mode'], tmp['err code'], tmp["color code"], tmp["description"]))
#                station_sub[stationSub]["downtime_error"][tmp['err code']] = {'count'      : 0,
#                                                                              'duration'   : 0,
#                                                                              'color'      : tmp["color code"],
...
#        if station_sub[stationSub]["downtime_error"]['N/A']['count'] == 0:
            if tmp['time mode'] == '1' or tmp['time mode'] == '0':
                station_sub[stationSub]["downtime_error"]['']['count'] = 1
...
#                                print modifiedMid
                    ...
                    if tmp['time mode'] == '0':
                        ...
                        station_sub[stationSub]["downtime_error"][tmp['err code']] = {'count'      : 0,
                                                                                      ...
                                                                                      'color'      : tmp["color code"],
                                                                                      'description': tmp["description"]}                                                                      
                elif not lastResult and tmp['time mode'] == '0':
                    station_sub[stationSub]["downtime_error"]["lastRec"].append((tmp['datetime'], tmp['time mode'], tmp['err code'], tmp["color code"], tmp["description"]))
                    station_sub[stationSub]["downtime_error"][tmp['err code']] = {'count'      : 0,
                                                                                  'duration'   : 0,
                                                                                  'color'      : tmp["color code"],
                                                                                  'description': tmp["description"]}
                elif not lastResult and tmp['time mode'] == '1':
        ...
        print tmp['time mode'] == '1', thisTime
        if lastMode == '0' and tmp['time mode'] == '1': # 0->1 count
            ...
            print "*&#", station_sub[stationSub]["downtime_error"]["lastRec"]
            station_sub[stationSub]["downtime_error"]["lastRec"] = []
        if tmp['time mode'] == '0':   
...
def AssembleTheResults(station_sub, startTime, retIP):
    ...
    for each in station_sub:
        ...
        del station_sub[each]["downtime_error"][""]
    for each in station_sub:
    ...
    for key in station_sub: 
        ...
        Subs[full_sub_name]["downtime"] = downtime
